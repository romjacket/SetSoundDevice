...
#cs
	Windows 10 Anniversary Update / August 2016
	trancexx > Try changing IID_IPolicyConfig:
	https://www.autoitscript.com/forum/topic/183977-after-win10-anniversary-update-setdefaultendpoint-change-default-sound-device-is-no-longer-working/#comment-1321327
...
#cs
	...
	https://msdn.microsoft.com/en-us/library/windows/desktop/dd370842%28v=vs.85%29.aspx
#ce

Global $eRender = 0
#cs
	0 = eRender = Audio rendering stream. Audio data flows from the application to the audio endpoint device, which renders the stream.
	1 = eCapture = Audio capture stream. Audio data flows from the audio endpoint device that captures the stream, to the application.
	2 = eAll = Audio rendering or capture stream. Audio data can flow either from the application to the audio endpoint device, or from the audio endpoint device to the application.

	https://msdn.microsoft.com/en-us/library/windows/desktop/dd371401%28v=vs.85%29.aspx
#ce
...
Func _SetChannelVolumeLevel($iChannel, $idB)
	...
	Else
		Return SetError(1, 0, 0)
	EndIf
EndFunc   ;==>_SetChannelVolumeLevel
...
Func _SetChannelVolumeLevelScalar($iChannel, $iVol)
	...
	Else
		Return SetError(1, 0, 0)
	EndIf
EndFunc   ;==>_SetChannelVolumeLevelScalar
...
Func _GetChannelVolumeLevel($iChannel)
	...
	Else
		Return SetError(1, 0, 0)
	EndIf
EndFunc   ;==>_GetChannelVolumeLevel
...
Func _GetChannelVolumeLevelScalar($iChannel)
	...
	Else
		Return SetError(1, 0, 0)
	EndIf
EndFunc   ;==>_GetChannelVolumeLevelScalar
